...
def make_html(
    ...
    q = """
        ...
        SELECT ?gssp ?t ?name ?mya ?uncertainty ?loc ?wkt ?bl ?ce ?s ?colour
        WHERE {
            {
                ?gssp 
                    a gssp:GSSP ;
            ...
            ?t
                skos:prefLabel ?name ;
                schema:color ?colour ;
                time:hasBeginning ?ageNode ;
            .
            ?ageNode gtsd:inMYA ?mya .
            OPTIONAL { ?ageNode schema:marginOfError ?uncertainty . }
    ...
    for r in g.query(q):
        ...
        rows[gssp]["uncertainty"] = r["uncertainty"]
    ...
    write_table(g, other_rows(g, set(rows)), others_output_path, "GSSAs & SABS", RANK.Eon)
    return output_path
...
def other_rows(g: Graph, included: set[str]) -> dict:
    ...
    for subject in sorted(g.objects(DATASET_IRI, SDO.hasPart), key=str):
        ...
        if (subject, RDF.type, GSSP.SABS) in g:
            name += " SABS"
        ages = [(age, beginning) for beginning in g.objects(interval, TIME.hasBeginning)
                for age in g.objects(beginning, GTSD.inMYA)]
        age, age_node = min(ages, key=lambda item: float(item[0])) if ages else (None, None)
        geometry = g.value(subject, GEO.hasGeometry)
        wkt = g.value(geometry, GEO.asWKT) if geometry is not None else None
        rows[str(subject)] = {
            "name": name,
            "interval": interval,
            "mya": str(age) if age is not None else "",
            "uncertainty": g.value(age_node, SDO.marginOfError) if age_node is not None else None,
...
def table_groups(g: Graph, rows: dict, group_rank: URIRef = RANK.Period) -> dict:
    groups = {}
    for subject, row in rows.items():
        path = interval_path(g, row["interval"])
        matches = [node for node in path if (node, GTS.rank, group_rank) in g]
        ...
        group = matches[-1] if matches else (eons[-1] if eons else path[0])
        groups.setdefault(group, []).append((subject, row, path))
    return groups


def write_table(g: Graph, rows: dict, output_path: Path, page_title: str,
                group_rank: URIRef = RANK.Period) -> Path:
    """Render tables grouped by rank, retaining coloured hierarchy headings."""
    ...
    with doc:
        h1(page_title)
        for group, entries in table_groups(g, rows, group_rank).items():
                                    ...
                                    with td():
                                        for index, item in enumerate(value):
                                            ...
                                            if isinstance(item, URIRef):
                                                ...
                                                text(str(item))
                                elif column == "mya" and row.get("uncertainty") is not None:
                                    td(f'{value} ± {row["uncertainty"]}')
...
def write_jekyll_html(
    ...
    labels = {
        ...
        "loc": "Location", "wkt": "Longitude, Latitude",
